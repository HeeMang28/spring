...
// 멤버벼수로 VO를 선언해준 다음
// 생성자에서 아이디(username), 비밀번호(password), 권한(auth), 최소 이 3항목을 입력해야
// 추후 VO대용으로 User의 자식클래스를 활용할 수 있습니다.

// praivte이 걸린 member를 외부에서 꺼내쓰기위해서 @Getter만 만들어줍니다.
@Getter
public class CustomUser extends User {
	...
	// CustomUser가 상속한 User클래스 기준으로 생성자를 설정ㅎ야
	// VO와 시큐리티를 연동할 수 있습니다.
	public CustomUser(MemberVO vo) {
		super(vo.getUserid(), // 첫 번째 파라미터로 아이디 꺼내주기 
				vo.getUserpw(), // 두 번째 파라미터로 비밀번호 꺼내주기
				
				// 세번째 파라미터는 List<AuthVO>를 SimpleGrantedAuthority로 변환해서
				// 입력해야 세팅이 됨
